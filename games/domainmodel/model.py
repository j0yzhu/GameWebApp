...
class Game:
    ...
    def release_date(self):
        return str(self.__release_date) if self.__release_date is not None else None
    ...
    def reviews(self) -> list:
        # raise DeprecationWarning("User.reviews is deprecated. Use get_reviews_by_user() in review_service instead.")
...
class User:
    def __init__(self, username: str, password: str):
        ...
        if isinstance(password, (bytes, str)) and len(password) > 0:
...
class Wish:
    def __init__(self, user: User, game: Game, wish_time: datetime=None):
        if not isinstance(user, User):
            raise ValueError("user must be an instance of User class")
        if not isinstance(game, Game):
            raise ValueError("game must be an instance of Game class")
        if not isinstance(wish_time, datetime) and wish_time is not None:
            raise ValueError("wish_time must be datetime object or None (defaults to current time)")

        self.__user = user
        self.__game = game
        self.__wish_time = wish_time if wish_time is not None else datetime.now()
    ...
    @property
    def wish_time(self) -> datetime:
        return self.__wish_time

    def __repr__(self):
        return f"Wish({self.__user} {self.__game} {self.__wish_time}"
